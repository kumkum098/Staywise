...
const seedDatabase = async () => {
  ...
  for (const [index, propertyData] of propertiesData.entries()) {
    ...
    const reviews = await Review.insertMany(comments.map((comment, reviewIndex) => {
      const base = property.livingScore;
      const adjustment = reviewIndex === 0 ? 0 : -0.5;
      const score = (value: number) => Math.max(1, Math.min(10, Number((value + adjustment).toFixed(1))));
      return { property: property._id, user: tenantUser._id, userName: reviewIndex === 0 ? 'Ananya Gupta' : 'Vikramaditya S.', userRole: 'Resident Student', cleanliness: score(base.cleanliness), internet: score(base.internet), food: score(base.food), noise: score(base.quietness), location: score(base.location), privacy: score(base.privacy), safety: score(base.safety), overall: score(base.overall), comment };
    }));
